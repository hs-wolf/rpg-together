...
import { SECURITY_NAME_BEARER, apiErrorHandler } from '@rpg-together/utilities'
 ...
 */
export async function expressAuthentication(request: Request,
  securityName: string,
  scopes: string[]): Promise<DecodedIdToken | undefined> {
 ...
 */
async function bearerAuth(request: Request,
  scopes: string[]): Promise<DecodedIdToken> {
  try {
    ...
    if (scopes.length && !scopes.includes(claims.role)) {
      throw new ApiError(
        ResponseCodes.UNAUTHORIZED,
        ResponseMessages.ROLE_NOT_AUTHORIZED,
      )
    }
    return claims
  }
