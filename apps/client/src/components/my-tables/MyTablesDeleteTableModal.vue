  ...
  <Transition name="fade">
    <div v-if="table" class="modal justify-center p-3">
      <div v-if="showConfirmCard" ref="confirmCardRef" class="card-primary gap-3">
        <h1 class="text-danger font-semibold">{{ $t('my-tables-delete-table.title') }}</h1>
        <p class="text-sm">{{ $t('my-tables-delete-table.confirmation') }}</p>
        <FormInput
          type="password"
      ...
      <div v-else ref="cardRef" class="card-primary gap-3">
        <h1 class="text-danger font-semibold">{{ $t('my-tables-delete-table.title') }}</h1>
        <p class="text-sm">{{ $t('my-tables-delete-table.warning') }}</p>
